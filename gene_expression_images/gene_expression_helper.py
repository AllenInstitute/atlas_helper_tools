...
def download_expression_images( dataset_id, output_directory, downsample_factor = 3 ) :
    ...
    for i in images :
        #print( i['section_number'], i['id'] )
        image_url  = "http://api.brain-map.org/api/v2/section_image_download/%d?downsample=%d&view=expression" % (i['id'],downsample_factor)
        image_path = os.path.join( output_directory, '%04d_%d.jpg' % (i['section_number'],dataset_id)    )
        urllib.request.urlretrieve(image_url, image_path)        
